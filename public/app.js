...
app.formResponseProcessor = function(formId,requestPayload,responsePayload){
  ...
  if(formId == 'accountCreate'){
    // Take the phoneNo and password, and use it to log the user in
    var newPayload = {
      'phoneNo' : requestPayload.phoneNo,
...
app.loadAccountEditPage = function(){
  // Get the phoneNo number from the current token, or log the user out if none is there
  var phoneNo = typeof(app.config.sessionToken.phoneNo) == 'string' ? app.config.sessionToken.phoneNo : false;
  if(phoneNo){
    // Fetch the user data
    var queryStringObject = {
      'phoneNo' : phoneNo
    ...
    app.client.request(undefined,'api/users','GET',queryStringObject,undefined,function(statusCode,responsePayload){
      if(statusCode == 200){
        ...
        document.querySelector("#accountEdit1 .displayPhoneInput").value = responsePayload.phoneNo;

        // Put the hidden phoneNo field into both forms
        var hiddenPhoneInputs = document.querySelectorAll("input.hiddenPhoneNumberInput");
        for(var i = 0; i < hiddenPhoneInputs.length; i++){
            hiddenPhoneInputs[i].value = responsePayload.phoneNo;
        }

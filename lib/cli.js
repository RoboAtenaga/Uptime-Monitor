    ...
    if(!err && userIds && userIds.length > 0){
      ...
      userIds.forEach(function(userId){
        _data.read('users',userId,function(err,userData){
          if(!err && userData){
            var line = 'Name: '+userData.firstName+' '+userData.lastName+' Phone: '+userData.phoneNo+' Checks: ';
...
cli.processInput = function(str){
  ...
  if(str){
    // Unique strings that identify the different unique questions allowed be the asked
    var uniqueInputs = [
      'man',
      'help',
      'exit',

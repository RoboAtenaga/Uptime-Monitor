/*
 * Worker-related tasks (background tasks)
 */
 // NOTE: This is a background worker, there is no requestor so we just log out mmessages
// Dependencies
...
workers.executeAllChecks = function(){
  // Get all checks
  _data.list('checks',function(err,checks){
    if(!err && checks && checks.length > 0){
      checks.forEach(function(checkFileName){
        // Read check data
        _data.read('checks',checkFileName,function(err, originalCheckData){
      ...
      debug("Error: Couldn't find any checks to process");
    }
  });
};

// Sanity-check data validator
...
workers.performCheck = function(originalCheckData){
  ...
  var isOutcomeSent = false;

  // Reconstruct the full url from the originalCheckData
  var parsedUrl = url.parse(originalCheckData.protocol+'://'+originalCheckData.url, true);
  // Get the hostname and path from reconstructed url
  ...
  var requestDetails = {
    ...
    'path' : path,
    'timeout' : originalCheckData.timeoutSeconds * 1000 // convert to milisecs
  };
  ...
  // Bind to the timeout event to handle when req times out
...
workers.init = function(){
  ...
  // Call the loop so checks will exceute later on
  workers.loop();
